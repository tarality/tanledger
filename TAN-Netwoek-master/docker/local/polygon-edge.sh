      ...
      case "$2" in 
          "ibft")
              if [ -f "$GENESIS_PATH" ]; then
                  ...
                  secrets=$("$Tan_EDGE_BIN" secrets init --insecure --num 4 --data-dir /data/data- --json)
                  echo "Secrets have been successfully generated"

                  rm -f /data/genesis.json

                  echo "Generating IBFT Genesis file..."
                  "$Tan_EDGE_BIN" genesis $CHAIN_CUSTOM_OPTIONS \
          ...
          "polybft")
              ...
              secrets=$("$Tan_EDGE_BIN" polybft-secrets init --insecure --num 4 --data-dir /data/data- --json)
              echo "Secrets have been successfully generated"
              ...
              "$Tan_EDGE_BIN" genesis $CHAIN_CUSTOM_OPTIONS \
                ...
                --native-token-config "TAN:MATIC:18:true:$(echo "$secrets" | jq -r '.[0] | .address')" \
                --bootnode "/dns4/node-1/tcp/1478/p2p/$(echo "$secrets" | jq -r '.[0] | .node_id')" \
                --bootnode "/dns4/node-2/tcp/1478/p2p/$(echo "$secrets" | jq -r '.[1] | .node_id')" \
                --bootnode "/dns4/node-3/tcp/1478/p2p/$(echo "$secrets" | jq -r '.[2] | .node_id')" \
                --bootnode "/dns4/node-4/tcp/1478/p2p/$(echo "$secrets" | jq -r '.[3] | .node_id')"
              ...
              "$Tan_EDGE_BIN" polybft stake-manager-deploy \
              ...
              "$Tan_EDGE_BIN" polybft whitelist-validators \
                --addresses ${addresses} \
              ...
              while [ $counter -le 4 ]; do
                ...
                "$Tan_EDGE_BIN" polybft register-validator \
                  --supernet-manager ${customSupernetManagerAddr} \
                  --data-dir /data/data-${counter} \
                  --jsonrpc http://rootchain:8545

                "$Tan_EDGE_BIN" polybft stake \
                  --data-dir /data/data-${counter} \
