...
import (
	...
	"github.com/tarality/tan-network/types/buildroot"
...
func (b *Blockchain) ComputeGenesis() error {
	...
	if ok {
		...
		// fmt.Println("----------line no 283---------------", b.config.Genesis)
...
func (b *Blockchain) executeBlockTransactions(block *types.Block) (*BlockResult, error) {
	...
	_, root, reward, iReward, err := txn.Commit()
	if err != nil {
		return nil, fmt.Errorf("failed to commit the state changes: %w", err)
	}
	header.BlockReward = reward
	header.InitialReward = iReward
	// fmt.Println("-----------line no 790----------", header.InitialReward)
...
func (b *Blockchain) CalculateBaseFee(parent *types.Header) uint64 {
	...
	baseFeeDelta := calcBaseFeeDelta(gasUsedDelta, parentGasTarget, parent.BaseFee)
	return common.Max(parent.BaseFee-baseFeeDelta, 1)
}
