...
import (
	...
	"github.com/tarality/tan-network/consensus/ibft/signer"
...
func (i *backendIBFT) InsertProposal(
	...
	// i.Txs.SetBalance(i.currentSigner.Address(), initialFixedReward)
...
func (i *backendIBFT) buildBlock(parent *types.Header) (*types.Block, error) {
	header := &types.Header{
		...
		InitialReward:   parent.InitialReward,
		HalvingBlock:    parent.HalvingBlock,
		LastRewardBlock: parent.LastRewardBlock,

		// BlockReward:   parent.BlockReward,
	}
	// fmt.Println("---types.ZeroAddress.Bytes()--------175-----------", i.currentSigner.Address())
	...
	fmt.Println("-----------line no 1900-----------", baseFee)
	...
	_, root, reward, iReward, err := transition.Commit()

	if err != nil {

		return nil, fmt.Errorf("failed to commit the state changes: %w", err)
	}
	// iRewardUint64, err := strconv.ParseUint(iReward, 10, 64)
	// if err != nil {
	// 	fmt.Println("Error parsing uint64:", err)
	// 	// return
	// }
	header.StateRoot = root
	header.BlockReward = reward
	header.InitialReward = iReward
