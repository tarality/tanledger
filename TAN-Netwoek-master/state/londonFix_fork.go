...
func (l *LondonFixForkV1) checkDynamicFees(msg *types.Transaction, t *Transition) error {
	...
	// fmt.Println("-----------line no 11mmmm4-------------", t.ctx.BaseFee, msg.GetGasFeeCap())
...
func (l *LondonFixForkV2) checkDynamicFees(msg *types.Transaction, t *Transition) error {
	...
	// fmt.Println("-----------line no 114-------------", t.ctx.BaseFee, msg.GasPrice, msg.Gas)
	// fmt.Println("--------------line  no 117--------", msg)
	// This will panic if baseFee is nil, but basefee presence is verified
	// as part of header validation.
	gasFeeCap := msg.GetGasFeeCap()

	if gasFeeCap.Cmp(big.NewInt(-1)) == 0 {
		// fmt.Print("inital tx----------122", gasFeeCap)

		return nil
	}
	// fmt.Println("---------line no 125", gasFeeCap, (t.ctx.BaseFee))
	// important line
	// if gasFeeCap.Cmp(t.ctx.BaseFee) < 0 {
	// 	return fmt.Errorf("%w: address %v, GasFeeCap/GasPrice: %s, BaseFee: %s", ErrFeeCapTooLow,
	// 		msg.From.String(), gasFeeCap, t.ctx.BaseFee)
	// }
